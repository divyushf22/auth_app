...
const issueToken = (details, expiresInHr:string = "1h", secretKey:string = "mysecret") : string =>  jwt.sign({ ...details }, secretKey, { expiresIn: expiresInHr });
...
const sendEmail = async (to:string, subject:string, text:string, from:string = "divyush@f22labs.com") => {
    ...
    mailTransporter.sendMail(mailDetails, function (err, data) : boolean {
        if (err) {
            throw err;
        } else {
            return true;
